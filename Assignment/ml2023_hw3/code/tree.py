...
class DecisionTree(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if best_split_id != None:
            ...
            # build child trees
            # TODO 2.3.1
            self.left = DecisionTree(self.split_loss_function,self.leaf_value_estimator,self.depth+1,self.min_sample,self.max_depth)
            self.right = DecisionTree(self.split_loss_function,self.leaf_value_estimator,self.depth+1,self.min_sample,self.max_depth)
...
def compute_entropy(label_array):
    ...
    # TODO 2.3.2
    zero_num = label_array[label_array == 0].shape[0]
    one_num = label_array[label_array == 1].shape[0]
    total_num = label_array.shape[0]
    entropy = -zero_num/total_num*np.log2(zero_num/total_num)-one_num/total_num*np.log2(one_num/total_num)
    return entropy
...
def compute_gini(label_array):
    ...
    one_num = label_array[label_array == 1].shape[0]
    total_num = label_array.shape[0]
    gini = 1-(zero_num/total_num)**2-(one_num/total_num)**2
    return gini
