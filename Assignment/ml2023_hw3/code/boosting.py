...
class GradientBoosting:
    ...
    def fit(self, train_data, train_target):
        ...
        for t in range(self.T):
            ...
            # TODO 3.5.4
            g_t = self.gradient_func(train_target, ft)
            rgs.fit(train_data, -g_t)
            self.ht.append(rgs)
    ...
    def predict(self, test_data):
        ...
        # TODO 3.5.5
        predict_target = np.zeros(test_data.shape[0])
        for t in range(self.T):
            predict_target += self.learning_rate * self.ht[t].predict(test_data)
...
if __name__ == '__main__':
    ...
    for idx, i, tt in zip(product([0, 1], [0, 1, 2]),
        ...
        axarr[idx[0], idx[1]].scatter(x_train[:, 0], x_train[:, 1], c=y_train_label, alpha=0.8)
        axarr[idx[0], idx[1]].set_title(tt)
    plt.savefig('output/GBM_logistic.jpg')
    ...
    plt.savefig("output/GBM_regression.jpg")
